...
\section{Zweifach-Zusammengang und st-Ordnungen}

\begin{definition}
  Ein \emph{2-fach-(knoten)zusammenhängender} Graph ist ein Graph $G=(V, E)$ für den jeder induzierte Teilgraph $G_{V\\v}$ für alle $v \in V$ zusammenhängend ist.
  Ist ein Graph $G=(V, E)$  nicht 2-fach-zusammenhängend, so nennt man die maximalen 2-fach-zusammenhängenden Teilgraphen die \emph{2-Zusammenhangskomponenten}. Die Zeugen $v \in V$, für die der induzierte Teilgraph $G_{V\\v}$ nicht zusammenhängend ist, nennt man \emph{Schnittknoten}. Kanten $e \in E$, für die $G' = (V, E\\e)$ nicht zusammenhängend ist, nennt man\emph{Brücken}.
\end{definition}

Ein Kreis ist ein 2-fach-zusammenhängender Graph, da durch Hinwegnahme eines Knotens ein zusammenhängender Weg entsteht. Ein Baum ist nicht 2-fach-knotenzusammenhängend, da durch Hinwegnahme der Wurzel, die Kinder nicht mehr zusammenhängend sind.

Die 2-Zusammenhangskomponenten eines Graphen bilden einen Baum.

Ein 2-fach-zusammenhängender Graph ist die Grundlage für eine weiteres Element das Zeichenalgorithmus:

\begin{definition}
  Eine $st$-Ordnung eines {2-fach-zusammenhängenden} Graphen $G = (V, E)$ und zweier Knoten $s, t \in V$ ist eine Anordnung aller Knoten $v_1, v_2, \dots, v_n \in V$, sodass $s = v_1$, $t = v_n$ und alle Knoten $v_i$ außer $s$ und $t$  einen Nachbarn $v_j$ mit $j < i$ und einen Nachbarn $v_k$ mit $k > i$ haben.
\end{definition}
